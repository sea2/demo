        ...
        OnPoiSearchListener, OnInfoWindowClickListener, OnClickListener
{
    ...
    protected void doSearchQuery(int poiType)
    {
        ...
        {
        case 0:
            query = new PoiSearch.Query("", "小区", currentCity);// 第一个参数表示搜索字符串，第二个参数表示poi搜索类型，第三个参数表示poi搜索区域（空字符串代表全国）
            break;
            
        case 1:
            query = new PoiSearch.Query("", "商务楼", currentCity);// 第一个参数表示搜索字符串，第二个参数表示poi搜索类型，第三个参数表示poi搜索区域（空字符串代表全国）
            break;
            
        case 2:
            query = new PoiSearch.Query("", "学校", currentCity);// 第一个参数表示搜索字符串，第二个参数表示poi搜索类型，第三个参数表示poi搜索区域（空字符串代表全国）
        ...
        if (mListener != null && aLocation != null)
        {
            ...
            if (!hasGot)
            {
                ...
                if ("".equals(keyWord))
                {
                    ...
                    ToastUtil.show(ChoosePlaceActivity.this, "对不起，查询不到您的信息");
                    return;
                }
                else
                {
        ...
        if (rCode == 0)
        {
            ...
            {// 搜索poi的结果
                ...
                {// 判断是否是同一条
                    ...
                    if (lastType != poiType)//判断poi上次搜索类型，如果当前poi类型跟上次不一样，则清空mList数据
                    {
                        mlist.clear();
                        aMap.clear();// 清理之前的图标
                        mlist.add(new Data(keyWord, "您的当前位置"));
